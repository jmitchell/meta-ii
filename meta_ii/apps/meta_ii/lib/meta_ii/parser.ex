defmodule MetaII.Parser do
  ...
  def identifier(src, trim \\ true) do
    re = ~S|(\p{L}\p{Xan}*)|
    re = if trim do
      ~S|\s*| <> re
    else
    ...
    case ~S|\A| <> re <> ~S|(.*\Z)| |> Regex.compile("s") do
      {:ok, re} ->
        case re |> Regex.run(src) do
          [_, ident, rest] ->
            {ident, rest}
          _ ->
            :error
